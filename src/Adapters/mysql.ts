...
export default class implements AdapterInterface {
  async getAllTables(db: knex, schemas: string[]): Promise<TableDefinition[]> {
    ...
    if (schemas.length > 0) query.whereIn("table_schema", schemas);
    return await query;
  }
  async getAllColumns(
    db: knex,
    ...
    const data = await db("information_schema.columns")
      .select("column_name AS name")
      .select(
        db.raw("(CASE WHEN is_nullable = 'NO' THEN 0 ELSE 1 END) AS isNullable")
      )
      .select(
        db.raw(
          "(SELECT CASE WHEN LOCATE('auto_increment', extra) <> 0 OR COLUMN_DEFAULT IS NOT NULL THEN 1 ELSE 0 END) AS isOptional"
        )
      )
